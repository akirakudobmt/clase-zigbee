  ...
  )
{
  ...
  case gZclCmdReportAttr_c:  
    BeeAppUpdateDevice(0, gZclUI_ZCLResponse_c, 0, NULL, pIndication);
...
)
{
  ...
  if ( pAddrInfo->txOptions & gApsTxOptionFragmentationRequested_c ) {
    ...
    return ZCL_CreateFragmentsFrame(command, frameControl, pTransactionId, pPayloadLen, pPayload);
  }
  else  
#endif
  {
...
{    
  uint8_t iTotalLen = *pPayloadLen + sizeof(zclFrame_t);
  void    *pThisMsgBuffer;
  uint8_t *pData;
  uint8_t fragLen;
  zclFrame_t *pFrame;
