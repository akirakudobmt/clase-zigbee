...
extern void   ZtcComm_Init(void);

#if (gZtcIncluded_d && gLpmIncluded_d)
...
#if gZtcIncluded_d
/*
  Returns currently selected channel via ZTC.
*/
void ZtcMsgGetChannelReqFunc(void)
{
  gZtcPacketToClient.structured.payload[sizeof(clientPacketStatus_t)] = 
    NlmeGetRequest(gNwkLogicalChannel_c);
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t)
...
void ZtcMsgGetApsDeviceKeyPairSet(void)
{
  ...
  /* Calculate the size to be send back */
...
void ZtcOtaSupportStartImageReqFunc(void) {
  uint32_t len;
  FLib_MemCpy(&len, gZtcPacketFromClient.structured.payload, 4);
  gZtcPacketToClient.structured.payload[0] = OTA_StartImage(OTA2Native32(len));
  
  gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));
}

void ZtcOtaSupportSetInternalFlashReqFunc(void) {
}

void ZtcOtaSupportPushImageChunkReqFunc(void) {
  uint16_t len;
  FLib_MemCpy(&len, &gZtcPacketFromClient.structured.payload, 2);  
  gZtcPacketToClient.structured.payload[0] = OTA_PushImageChunk(&gZtcPacketFromClient.structured.payload[2], 
                                                                 OTA2Native16(len), NULL);  
  gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));
}

void ZtcOtaSupportCommitImageReqFunc(void) {

   gZtcPacketToClient.structured.payload[0] = OTA_CommitImage(&gZtcPacketFromClient.structured.payload[3]);  
  gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));
}
...
{
  gZtcPacketToClient.structured.payload[0] = GetMaxApplicationPayload((zbApsdeDataReq_t *)&gZtcPacketFromClient.structured.payload[0]);
}
...
void ZtcWriteMemoryBlock(void) 
{
  ...
  nBytes = gZtcPacketFromClient.structured.payload[sizeofPointer];
  pIn = &gZtcPacketFromClient.structured.payload[sizeofPointer + 1];
