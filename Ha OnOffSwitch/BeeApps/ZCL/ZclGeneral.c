...
/******************************
  Temperature Configuration Cluster Data
...
//    Set2Bytes(pReq->addrInfo.aClusterId, gZclClusterAlarm_c);	
...
(
zclAlarmInformation_ResetAllAlarms_t *pReq
)
{
...
//zbStatus_t Alarms_ResetAllAlarms(zclCmd ¿?¿? *pReq){}
...
(
zclCmdAlarmInformation_GetAlarmResponse_t *pReq
)
{
   // pReq->zclTransactionId = gZclTransactionId++;
//    Set2Bytes(pReq->addrInfo.aClusterId, gZclClusterAlarm_c);	
    return ZCL_SendClientReqSeqPassed(gAlarmClusterTxCommandID_GetAlarmResponse_c,0,(zclGenericReq_t *)pReq);
