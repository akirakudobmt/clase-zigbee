...
)
{
  ...
  /* Keep the highest value to track the channle with less amount of networks. */
  uint8_t channelWithLessNetworks = 0xFF;
  ...
  */
  for (i = 0; i < gMaxDiscoveryTableEntries; i++)
  {
    ...
    /*
      ...
      And keep track of the amount of netwroksper channel.
    */
  ...
  {
    /*
      pChannelList represents the Channels Specified by Upper layer (e.g Fromation Request)
      Check if channel was scanned.
    */
    if ( pChannelList[ iIndex ] & bitMask )
    {
      if (aNetworkCount[iListIndex - i] < channelWithLessNetworks)
      {
        channelWithLessNetworks = (iListIndex - i);
      }
    }
  ...
  return (channelWithLessNetworks + 11);
}
...
)
{
  ...
  /*
    If there is another network in this channel with our extended pan ID, then, there
    is a conflict.
  */
  if(CheckForExtendedPanIdConflict(channel))
  {
    return TRUE;
  }
...
/**********************************************************************************
*
* When a network update command is received this function will be called to notify
* the application that the pan id will change.
*
**********************************************************************************/
void PanIdConflict_PanIdChangePending()
{
...
#endif
  {
  (void)SetPermitJoinAttribute(PermitJoinOff);
  }
#endif
}
...
/**********************************************************************************
*
* When the panID is changed as a result to pan id conflict resolution this function is called
*
**********************************************************************************/
void PanIdConflict_PanIdChanged()
{
#if ( gCoordinatorCapability_d || gRouterCapability_d || gComboDeviceCapability_d)
#if gComboDeviceCapability_d
  if(NlmeGetRequest(gDevType_c) != gEndDevice_c)
#endif
  {
  (void)SetPermitJoinAttribute(PermitJoinOn);
  }
#endif
}
